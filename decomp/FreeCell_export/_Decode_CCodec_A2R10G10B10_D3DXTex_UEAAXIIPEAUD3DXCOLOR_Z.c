void __fastcall D3DXTex::CCodec_A2R10G10B10::Decode(
        D3DXTex::CCodec_A2R10G10B10 *this,
        int a2,
        int a3,
        struct D3DXCOLOR *a4)
{
  float *v4; // r10
  _DWORD *v5; // rdx
  _DWORD *i; // r8
  int v7; // eax

  v4 = (float *)a4;
  v5 = (_DWORD *)(*((_QWORD *)this + 5)
                + (unsigned int)(a3 * *((_DWORD *)this + 33))
                + (unsigned __int64)(unsigned int)(a2 * *((_DWORD *)this + 32)));
  for ( i = &v5[*((unsigned int *)this + 34)]; v5 < i; *(v4 - 1) = (float)v7 * 0.33333334 )
  {
    *v4 = (float)((*v5 >> 20) & 0x3FF) * 0.00097751711;
    v4 += 4;
    *(v4 - 3) = (float)((*v5 >> 10) & 0x3FF) * 0.00097751711;
    *(v4 - 2) = (float)(*v5 & 0x3FF) * 0.00097751711;
    v7 = *v5++ >> 30;
  }
  if ( *((_DWORD *)this + 7) )
    D3DXTex::CCodec::ColorKey(this, (struct D3DXCOLOR *)&v4[-4 * *((unsigned int *)this + 34)]);
  if ( *((_DWORD *)this + 5) )
    D3DXTex::CCodec::DecodeGamma22(this, (struct D3DXCOLOR *)&v4[-4 * *((unsigned int *)this + 34)]);
}
