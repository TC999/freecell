__int64 __fastcall MaterialAdditive::OnDeviceLost(MaterialAdditive *this)
{
  signed int v1; // eax
  unsigned int v2; // ebx
  ...
  if ( v1 < 0 )
  {
    ...
    LODWORD(v6) = v4;
    Log(2u, 419, "MaterialAlpha.cpp", L"Unable to recreate the vertex buffer: %x(%s)", v6, HResult);
  }
  return v2;
}
